...
const getLatestSectionSummaries = (rawSections: any[] | undefined) => {
  ...
  rawSections.forEach((s) => {
    ...
    const existingTs =
      existing && existing.lastCalculated
        ? Date.parse(existing.lastCalculated)
        : 0;
...
const normalizeCalculationBreakdown = (raw: any) => {
  ...
  const sectionCalculations = (inputSections || []).map(
    (s: any, idx: number) => {
      const items = (s.riskItems || []).map((ri: any, i: number) => ({
        ...
        actualPremiumFormula:
          ri.actualPremiumFormula ?? ri.actualPremiumFormula,
      ...
      return {
        ...
        sectionName:
          s.sectionName ?? s.sectionDisplayName ?? `Section ${idx + 1}`,
        location: s.location,
        riskItemCalculations: {
          ...
          totals: {
            totalActualValue:
              s.sectionSumInsured ??
              items.reduce(
                (a: number, it: any) => a + (it.actualValue || 0),
                0
              ),
            totalSharePremium:
              s.sectionGrossPremium ??
              items.reduce(
                (a: number, it: any) => a + (it.actualPremium || 0),
                0
              ),
  ...
  const finalResults = raw.finalResults ?? {
    totalSumInsured: inputSections.reduce(
      (acc: number, s: any) => acc + (s.sectionSumInsured || 0),
      0
    ),
    totalPremium: inputSections.reduce(
      (acc: number, s: any) => acc + (s.sectionGrossPremium || 0),
      0
    ),
    netPremium: inputSections.reduce(
      (acc: number, s: any) => acc + (s.sectionNetPremium || 0),
      0
    ),
  };
  ...
  return {
    ...
    calculationSteps: {
      sectionCalculations,
      proRataCalculations:
        raw.inputs?.proRataCalculations ?? raw.proRataCalculations,
    },
    finalResults,
...
const QuoteCreator = () => {
  ...
  const [normalizedBreakdown, setNormalizedBreakdown] = useState<any | null>(
    null
  );
  const [aggregateTotals, setAggregateTotals] =
    useState<aggregateTotals | null>(null);
  const [sectionsLoading, setSectionsLoading] = useState(false);
  const [localSectionsSummary, setLocalSectionsSummary] = useState<any | null>(
    null
  );
  const [payloadSection, setPayloadSection] = useState<any[] | null>([]);
  const [localSections, setLocalSections] = useState<QuoteSection[] | null>(
    null
  );
  const [calculatedRiskMap, setCalculatedRiskMap] = useState<
    Record<string, any[]>
  >({});
  const [editedSections, setEditedSections] = useState<number[]>([]);
  const [proposalAdjustmentsResult, setProposalAdjustmentsResult] = useState<
    any | null
  >(null);
      ...
      setAdjustments({
        otherDiscountsRate:
          calculationBreakdown?.inputs?.proposalAdjustments
            .otherDiscountsRate || 0,
        otherLoadingsRate:
          calculationBreakdown?.inputs?.proposalAdjustments.otherLoadingsRate ||
          0,
      });
  ...
  const currentProduct = products.find(
    (p) => p.productID === proposalProductId
  );
  ...
  const handleSaveSection = async (fullsection: any) => {
    ...
    setPayloadSection((prev) => {
      ...
      if (editingSectionId == null) {
        // <-- check null/undefined directly
        list.push(fullsection);
      } else {
        setEditedSections((prev) =>
          prev.includes(Number(editingSectionId))
            ? prev
            : [...prev, Number(editingSectionId)]
        );
  ...
  const handleDeleteSection = async (sectionId: number) => {
    if (!window.confirm("Are you sure you want to delete this section?"))
      return;
    setPayloadSection((prev) =>
      prev ? prev.filter((s, index: any) => index !== sectionId) : null
    );

    setLocalSections((prev) =>
      prev ? prev.filter((s, index) => index !== sectionId) : null
    );
    setLocalSectionsSummary((prev: any) => {
      if (!prev) return prev;
      return {
  ...
  const handleReceiveFullRiskArray = (
    sectionID: string,
    fullRiskArray: any[]
  ) => {
    setCalculatedRiskMap((prev) => ({ ...prev, [sectionID]: fullRiskArray }));

    const existsInLocalSections = !!(
      localSections && localSections.find((s) => s.sectionID === sectionID)
    );
    ...
    if (existsInLocalSections || existsInSummary) {
      setLocalSectionsSummary((prev: any) => {
        const copy = prev ? { ...prev } : { sections: [] };
        const sectionsArr = Array.isArray(copy.sections)
          ? [...copy.sections]
          : [];
  ...
  const buildMultiSectionPayload = () => {
    const authoritativeSections: any[] = localSections
      ...
      : localSectionsSummary?.sections &&
        localSectionsSummary.sections.length > 0
      ? localSectionsSummary.sections
  ...
  const calculateMultiSectionAggregateLocal = async () => {
    ...
    try {
      ...
      const resultAction = await dispatch(
        calculateMultiSectionAggregate(payload)
      );
  ...
  const handleApplyProposalAdjustments = async () => {
    ...
    try {
      // @ts-ignore
      const res = await (
        dispatch(applyProposalAdjustments(payload)) as any
      ).unwrap?.();
  ...
  const netPremium =
    proposalAdjustmentsResult?.netPremiumDue ||
    calculationBreakdown?.calculationSteps?.proRataCalculations
      .netPremiumBeforeProRata;
  useEffect(() => {
    if (netPremium) handleCalculateProRata();
  }, []);
  ...
  const handleCalculateProRata = async () => {
    if (!netPremium || Number(netPremium) <= 0) {
      toast({
        description:
          "No net premium available to pro-rate. Run proposal adjustments or a calculation first.",
        variant: "warning",
        duration: 2000,
      });
  ...
  const handleCalculate = async () => {
    ...
    const sectionsToCalculate = payloadSection?.map(
      ({ lastCalculated, proportionRate, ...rest }) => ({
        ...rest,
    ...
    const breakdownSections = (
      calculationBreakdown?.inputs?.sectionInputs || []
    )
      .filter((si: any, index) => !editedSections.includes(index))
      .map((si: any) => {
        return {
          ...
          riskItems: (si.riskItems || []).map((it: any) => ({
            ...
            netPremiumAfterDiscounts:
              it.netPremiumAfterDiscounts ?? it.stockNetPremium ?? 0,
            ...
            totalSumInsured:
              it.totalSumInsured ??
              it.stockSumInsured ??
              (it.actualValue ? it.actualValue * (it.multiplyRate ?? 1) : 0),
          })),

          // section-level rates: look into sectionAdjustments for rates first, else fallback to 0 or si fields
          specialDiscountRate:
            si.sectionAdjustments?.discountsApplied?.find(
              (d: any) => d.name === "Special Discount"
            )?.rate ??
            si.specialDiscountRate ??
            0,
          deductibleDiscountRate:
            si.sectionAdjustments?.discountsApplied?.find(
              (d: any) => d.name === "Deductible Discount"
            )?.rate ??
            si.deductibleDiscountRate ??
            0,
          spreadDiscountRate:
            si.sectionAdjustments?.discountsApplied?.find(
              (d: any) => d.name === "Spread Discount"
            )?.rate ??
            si.spreadDiscountRate ??
            0,
          ltaDiscountRate:
            si.sectionAdjustments?.discountsApplied?.find(
              (d: any) => d.name === "LTA Discount"
            )?.rate ??
            si.ltaDiscountRate ??
            0,
          otherDiscountsRate: si.otherDiscountsRate ?? 0,
          theftLoadingRate:
            si.sectionAdjustments?.loadingsApplied?.find(
              (d: any) => d.name === "Theft Loading"
            )?.rate ??
            si.theftLoadingRate ??
            0,
          srccLoadingRate:
            si.sectionAdjustments?.loadingsApplied?.find(
              (d: any) => d.name === "SRCC Loading"
            )?.rate ??
            si.srccLoadingRate ??
            0,
          otherLoading2Rate:
            si.sectionAdjustments?.loadingsApplied?.find(
              (d: any) => d.name === "Other Loading 2"
            )?.rate ??
            si.otherLoading2Rate ??
            0,
        };
      });
    ...
    try {
      ...
      await Promise.resolve(dispatch(getSectionsSummary(proposalNo) as any))
        .then((res: any) => {
          ...
          if (payload && sectionsFetchIdRef.current === fetchId)
            setLocalSectionsSummary(payload);
        })
        .catch(() => {})
        .finally(() => {
  ...
  return (
    <div className="qc-container">
      <div className="qc-header">
        <div>
          ...
          <p className="qc-proposal-info">
            Proposal: {currentProposal.proposalNo} | Product:{" "}
        ...
        <div className="qc-header-actions">
          ...
          <Button
            ...
            disabled={
              loading?.calculate || !localSections || localSections.length === 0
            }
          >
            {loading?.calculate
              ? "Calculating..."
      ...
      {error?.calculate && (
        <div className="qc-error-message">{error.calculate}</div>
      )}
      {success?.calculate && (
        <div className="qc-success-message">
      ...
      <div className="qc-content">
        ...
        <div className="qc-sections-panel">
          ...
          {(() => {
            const summaryList = localSectionsSummary?.sections
              ? getLatestSectionSummaries(localSectionsSummary)
              : null;
            const listToRender =
              localSections && localSections.length > 0
                ? localSections
                ...
                >
                  ...
                  <tbody>
                    {listToRender.map((section: any, index) => {
                      ...
                      const sumInsured =
                        section.sectionSumInsured ??
                        section.sectionSumInsured ??
                        0;
                      const premium =
                        section.sectionGrossPremium ??
                        section.sectionPremium ??
                        0;
                        ...
                        <tr key={id} className="qc-section-row">
                          ...
                          <td>
                            <div style={{ display: "flex", gap: 8 }}>
                              <Button
                                onClick={() => handleEditSection(String(index))}
                                size="sm"
                                variant="outline"
                              >
                                Edit
                ...
                <div className="qc-results-grid">
                  <div className="qc-result-item">
                    ...
                    <div className="qc-result-value">
                      {formatCurrency(aggregateTotals.totalSumInsured)}
                    </div>
                  </div>
                  <div className="qc-result-item">
                    <Label>Total Aggregate Premium</Label>
                    <div className="qc-result-value">
                      {formatCurrency(aggregateTotals?.totalAggregatePremium)}
                    </div>
                  </div>
                  <div className="qc-result-item">
                    <Label>Section Count</Label>
                    <div className="qc-result-value">
                      {aggregateTotals?.sectionCount}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="qc-adjustments-grid">
              <div className="qc-adjustment-section">
                ...
                <div className="qc-adjustment-field">
                  <Label htmlFor="otherDiscountsRate">
                    Other Discounts (%)
                  </Label>
                  <Input
              ...
              <div className="qc-adjustment-section">
                ...
                <div className="qc-adjustment-field">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setAdjustments((prev) => ({
                        ...prev,
                        otherLoadingsRate: Number(e.target.value),
                      }))
                    }
                  />
                </div>
                <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
                  <Button
                    onClick={handleApplyProposalAdjustments}
                    size="sm"
                    variant="outline"
                  >
 ...
 */}{" "}
          ...
          {(calculationBreakdown?.calculationSteps?.proposalAdjustments
            ?.discountsApplied.length > 0 ||
            calculationBreakdown?.calculationSteps?.proposalAdjustments
              ?.loadingsApplied.length > 0 ||
              ...
              <div className="card-body two-column">
                <div className="summary-column">
                  <div className="summary-row">
                    ...
                    <div className="value">
                      {formatCurrency(
                        proposalAdjustmentsResult?.startingPremium ||
                          calculationBreakdown?.calculationSteps
                            ?.proposalAdjustments?.startingPremium ||
                          0
                      )}
                    </div>
                  </div>
                  ...
                  <div className="summary-row">
                    ...
                    <div className="value highlight">
                      {formatCurrency(
                        proposalAdjustmentsResult?.netPremiumDue ||
                          calculationBreakdown?.calculationSteps
                            ?.proposalAdjustments?.finalNetPremium ||
                          0
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mini-grid">
                    <div className="mini-item">
                      ...
                      <div className="mini-value">
                        {formatCurrency(
                          proposalAdjustmentsResult?.otherDiscountsAmount ||
                            calculationBreakdown?.calculationSteps
                              ?.proposalAdjustments?.discountsApplied[0]
                              ?.amount ||
                            0
                        )}
                      </div>
                      <div className="mini-value">
                        Net Amount:
                        {formatCurrency(
                          proposalAdjustmentsResult?.otherDiscountsNetAmount ||
                            0
                        )}
                      </div>
                    </div>
                ...
                <div className="summary-column">
                  ...
                  <div className="mini-grid">
                    <div className="mini-item">
                      ...
                      <div className="mini-value">
                        {formatCurrency(
                          proposalAdjustmentsResult?.otherLoadingsAmount ||
                            calculationBreakdown?.calculationSteps
                              ?.proposalAdjustments?.loadingsApplied[0]
                              ?.amount ||
                            0
                        )}
                      </div>
                      <div className="mini-value">
                        Net Amount:
                        {formatCurrency(
                          proposalAdjustmentsResult?.otherLoadingsNetAmount || 0
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="footer-note">
                    <small className="muted">
                      {proposalAdjustmentsResult?.message ??
                        "Proposal-level adjustments applied."}
                    </small>
                  </div>
                </div>
              </div>
              ...
              <div className="card-body pro-rata-grid">
                <div className="pr-row">
                  ...
                  <div className="pr-value">
                    {formatCurrency(
                      proRataResult?.netPremiumDue ||
                        calculationBreakdown?.calculationSteps
                          ?.proRataCalculations.netPremiumBeforeProRata ||
                        0
                    )}
                  </div>
                </div>
                ...
                <div className="pr-row">
                  ...
                  <div className="pr-value">
                    {formatCurrency(
                      proRataResult?.proRataPremium ||
                        calculationBreakdown?.calculationSteps
                          ?.proRataCalculations.proRataPremium ||
                        0
                    )}
                  </div>
                </div>
                ...
                <div className="pr-row">
                  ...
                  <div className="pr-value">
                    {formatCurrency(
                      proRataResult?.premiumDue ||
                        calculationBreakdown?.calculationSteps
                          ?.proRataCalculations.finalPremiumDue ||
                        0
                    )}
                  </div>
                </div>
                ...
                <div className="pr-row">
                  ...
                  <div className="pr-value">
                    {proRataResult?.coverDays ?? coverDays ?? "—"}
                  </div>
                </div>
              </div>
            </div>
            ...
            {!showDetailedBreakdown ? (
              <>
                <div className="qc-results-grid">
                  <div className="qc-result-item">
                    <Label>Total Sum Insured</Label>
                    <div className="qc-result-value">
                      {formatCurrency(currentCalculation.totalSumInsured)}
                    </div>
                  </div>
                  <div className="qc-result-item">
                    <Label>Total Premium</Label>
                    <div className="qc-result-value">
                      {formatCurrency(currentCalculation.totalPremium)}
                    </div>
                  </div>
                  <div className="qc-result-item">
                    <Label>Net Premium</Label>
                    <div className="qc-result-value">
                      {formatCurrency(currentCalculation.netPremium)}
                    </div>
                  </div>
                  <div className="qc-result-item">
                    <Label>Pro-Rata Premium</Label>
                    <div className="qc-result-value">
                      {formatCurrency(currentCalculation.proRataPremium)}
                    </div>
                  </div>
                  <div className="qc-result-item">
                    <Label>Share Sum Insured</Label>
                    <div className="qc-result-value">
                      {formatCurrency(currentCalculation.shareSumInsured)}
                    </div>
                  </div>
                  <div className="qc-result-item">
                    <Label>Share Premium</Label>
                    <div className="qc-result-value">
                      {formatCurrency(currentCalculation.sharePremium)}
                    </div>
                  </div>
                </div>

                {localSectionsSummary == [] &&
                  localSectionsSummary.length > 0 && (
                    <div className="qc-sections-summary">
                          ...
                          <tbody>
                            {localSectionsSummary.map((section: any) => (
                              <tr key={section.sectionID}>
                                ...
                                <td className="right">
                                  {formatCurrency(section.sectionSumInsured)}
                                </td>
                                <td className="right">
                                  {formatCurrency(section.sectionGrossPremium)}
                                </td>
                                <td className="right highlight">
                                  {formatCurrency(
                                    section.netPremium ??
                                      section.sectionNetPremium ??
                                      0
                                  )}
                                </td>
                                <td className="center">
                                  {section.lastCalculated
                                    ? new Date(
                                        section.lastCalculated
                                      ).toLocaleString()
                                    : "N/A"}
                ...
                {currentCalculation.validationErrors &&
                  currentCalculation.validationErrors.length > 0 && (
                    <div className="qc-validation-errors">
                      <h4>Validation Issues:</h4>
                      <ul>
                        {currentCalculation.validationErrors.map(
                          (err: any, idx: number) => (
                            <li key={idx}>{err}</li>
                          )
                        )}
              ...
              breakdown && (
                <div className="qc-detailed-breakdown">
                    ...
                    <div className="qc-info-grid">
                      <div className="qc-info-item">
                        ...
                        <div className="qc-info-value">
                          {breakdown.calculatedOn
                            ? new Date(breakdown.calculatedOn).toLocaleString()
                            : "N/A"}
                        </div>
                      </div>
                      <div className="qc-info-item">
                        ...
                        <div className="qc-info-value">
                          {breakdown.calculatedBy || "N/A"}
                        </div>
                      </div>
                      <div className="qc-info-item">
                        <Label>Type</Label>
                        <div className="qc-info-value">
                          {breakdown.calculationType || "N/A"}
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="qc-breakdown-section">
                      ...
                      <div className="qc-info-grid">
                        <div className="qc-info-item">
                          ...
                          <div className="qc-info-value">
                            {breakdown.inputs.proportionRate ?? "N/A"}%
                          </div>
                        </div>
                        <div className="qc-info-item">
                          <Label>Exchange Rate</Label>
                          <div className="qc-info-value">
                            {breakdown.inputs.exchangeRate ?? "N/A"}
                          </div>
                        </div>
                        <div className="qc-info-item">
                          <Label>Currency</Label>
                          <div className="qc-info-value">
                            {breakdown.inputs.currency ?? "N/A"}
                          </div>
                        </div>
                        <div className="qc-info-item">
                          <Label>Cover Days</Label>
                          <div className="qc-info-value">
                            {breakdown.inputs.coverDays ?? "N/A"}
                          </div>
                        </div>
                        <div className="qc-info-item">
                          <Label>Start</Label>
                          <div className="qc-info-value">
                            {breakdown.inputs.startDate
                              ? new Date(
                                  breakdown.inputs.startDate
                                ).toLocaleString()
                              : "N/A"}
                  ...
                  {breakdown.calculationSteps?.sectionCalculations?.map(
                    (sectionCalc: any) => (
                      <div
                        key={sectionCalc.sectionID}
                        className="qc-breakdown-section"
                      >
                        ...
                        <div className="qc-items-breakdown">
                          ...
                          <div className="qc-items-table">
                            <table>
                              ...
                              <tbody>
                                {sectionCalc.riskItemCalculations?.items?.map(
                                  (item: any) => (
                                    <tr
                                      key={`${item.smiCode || ""}-${
                                        item.itemNo
                                    ...
                                    >
                                      ...
                                      <td className="qc-formula-cell">
                                        {item.actualPremiumFormula}
                                      </td>
                                      <td>
                                        {formatCurrency(item.actualPremium)}
                                      </td>
                                      <td>
                              ...
                              <tfoot>
                                <tr className="qc-totals-row">
                                  ...
                                  <td>
                                    <strong>
                                      {formatCurrency(
                                        sectionCalc.riskItemCalculations?.totals
                                          ?.totalActualValue || 0
                                      )}
                                    </strong>
                                  </td>
                                  ...
                                  <td className="qc-highlight">
                                    <strong>
                                      {formatCurrency(
                                        sectionCalc.riskItemCalculations?.totals
                                          ?.totalNetPremiumAfterDiscounts || 0
                                      )}
                                    </strong>
                                  </td>
                                </tr>
                        ...
                        {sectionCalc.sectionAdjustments && (
                          <div className="qc-section-adjustments">
                            ...
                            <div className="qc-adjustments-flow">
                              ...
                              {sectionCalc.sectionAdjustments.loadingsApplied?.map(
                                (loading: any, idx: number) => (
                                  <div
                                    key={idx}
                                    className="qc-adjustment-step qc-loading"
                                  >
                                    <Label>{loading.name}</Label>
                                    <div className="qc-value">
                                      +{formatCurrency(loading.amount)}
                                    </div>
                                  </div>
                                )
                              )}
                              {sectionCalc.sectionAdjustments.discountsApplied?.map(
                                (discount: any, idx: number) => (
                                  <div
                                    key={idx}
                                    className="qc-adjustment-step qc-discount"
                                  >
                    ...
                    <div className="qc-breakdown-section qc-final-results">
                      ...
                      <div className="qc-results-grid">
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {formatCurrency(
                              breakdown.finalResults.totalSumInsured
                            )}
                          </div>
                        </div>
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {formatCurrency(
                              breakdown.finalResults.totalPremium
                            )}
                          </div>
                        </div>
                        <div className="qc-result-item">
                          <Label>Net Premium</Label>
                          <div className="qc-result-value">
                            {formatCurrency(breakdown.finalResults.netPremium)}
                          </div>
                        </div>
                        <div className="qc-result-item">
                          <Label>Pro-Rata Premium</Label>
                          <div className="qc-result-value">
                            {formatCurrency(
                              breakdown.finalResults.proRataPremium
                            )}
                          </div>
                        </div>
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {formatCurrency(
                              breakdown.finalResults.shareSumInsured
                            )}
                          </div>
                        </div>
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {formatCurrency(
                              breakdown.finalResults.sharePremium
                            )}
                          </div>
                        </div>
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {formatCurrency(
                              breakdown.finalResults.foreignSumInsured
                            )}{" "}
                        ...
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {formatCurrency(
                              breakdown.finalResults.foreignPremium
                            )}{" "}
                            {breakdown.finalResults.foreignCurrency}
                          </div>
                        </div>
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {breakdown.finalResults.overallPremiumRate?.toFixed(
                              4
                            )}
                            %
                          </div>
                        ...
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {breakdown.finalResults.effectiveDiscountRate?.toFixed(
                              2
                            )}
                            %
                          </div>
                        ...
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {breakdown.finalResults.effectiveLoadingRate?.toFixed(
                              2
                            )}
                            %
                          </div>
                        ...
                        <div className="qc-result-item">
                          ...
                          <div className="qc-result-value">
                            {breakdown.finalResults.proportionShare}%
                          </div>
                        </div>
                      </div>
                    </div>
